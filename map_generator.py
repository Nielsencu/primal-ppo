...
def generateWarehouse(num_block=[-1,-1], length=40, shelfSize=5,lbRatio=2/3,freeSpaceRatio=1/3):
    if(num_block[0]!=-1):
    ...
    for i in range(freeSpace, freeSpace+noShelves*(shelfSize+1), shelfSize+1):
        for j in range(1, length-1, 2):
            world[j,i:i+shelfSize] = -1
    return world

def house_generator(env_size=(20, 40), obstacle_ratio=10, remove_edge_ratio=6):
    min_size = env_size[0]
    max_size = env_size[1]
    world_size = np.random.randint(min_size, max_size, 1)
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        # create a warehouse
        world = random_generator(SIZE_O=(10, 40), PROB_O=(0, .3))
        # world = warehouse_generator(num_block=(2, 2), num_shelves=(4, 2), ent_location=10, ent_position="top", entrance_size=3)
        # world = maze_generator(env_size=(30, 40), wall_components=(7, 8), obstacle_density=(0, .6))
        # world = house_generator(env_size=(10, 40))
